...
const UpdateListing = () => {
  ...
  const storeImage = async (file) => {
    ...
    const response = await uploadImageToCloudinary(data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.imageUrls.length < 1)
        toast.error("You must upload at least one image!", { autoClose: 3000 });
      else if (+formData.regularPrice < +formData.discountPrice)
        toast.error("Discount price must be lower than regular price", { autoClose: 3000 });
      else {
        setLoading(true);
        ...
        toast.success('Listing Updated Successfully!', { autoClose: 3000 });
